...
class Account:
    ...
    def assess_interest_and_fees(self):
        ...
        all_transacions = self._transactions
        latest_transaction = max(all_transacions)

        for transaction in all_transacions:
            if (transaction.in_same_month(latest_transaction) and transaction.is_exempt()):
                raise TransactionSequenceError (latest_transaction.get_date())
...
class SavingsAccount(Account):
    ...
    def _check_limits(self, t1):
        ...
        if (num_today >= self._daily_limit or num_this_month >= self._monthly_limit):
            raise TransactionLimitError(num_today, num_this_month)
